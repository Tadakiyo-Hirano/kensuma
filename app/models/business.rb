class Business < ApplicationRecord
  ...
  enum business_health_insurance_status: { join: 0, not_join: 1, not_coverd: 2 }               # 健康保険(加入状況)
  enum business_welfare_pension_insurance_join_status: { join: 0, not_join: 1, not_coverd: 2 } # 厚生年金保険(加入状況)
  enum business_pension_insurance_join_status: { welfare: 0, national: 1, recipient: 2 }       # 年金保険(加入状況)
  enum business_employment_insurance_join_status: { join: 0, not_join: 1, not_coverd: 2 }      # 雇用保険(加入状況)
  enum business_retirement_benefit_mutual_aid_status: { available: 0, not_available: 1 }       # 退職金共済制度(加入状況)
