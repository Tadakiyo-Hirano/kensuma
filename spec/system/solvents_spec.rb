...
RSpec.xdescribe 'Solvents', type: :system do
  ...
  let(:solvent_a) { create(:solvent, name: 'TESTシンナー', maker: 'TESTメーカー', classification: 'シンナー', ingredients: 'トルエン・キシレン', business: business) }
  ...
  describe '有機溶剤関連' do
    ...
    context '有機溶剤情報登録(正常系)' do
      it '新規登録したあと詳細画面へ遷移すること(正常系)' do
        ...
        fill_in 'solvent[name]', with: solvent_a.name # 商品名
